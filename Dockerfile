...
FROM node:20.18.1-bookworm-slim AS npm-install
WORKDIR /srv/src/kpi
...
#                              #
################################
FROM node:20.18.1-bookworm-slim AS webpack-build-prod
WORKDIR /srv/src/kpi
RUN chown node:node .
...
ARG DATAMOVIN_IMAGE_CREATED="1970-01-01T00:00:00Z"
ARG DATAMOVIN_IMAGE_REVISION="local"
ARG DATAMOVIN_IMAGE_VERSION="2.026.21a-datamovin.1"

LABEL org.opencontainers.image.title="DataMovin KPI" \
      org.opencontainers.image.description="DataMovin field data application" \
      org.opencontainers.image.created="${DATAMOVIN_IMAGE_CREATED}" \
      org.opencontainers.image.revision="${DATAMOVIN_IMAGE_REVISION}" \
      org.opencontainers.image.source="https://github.com/proyectomovin/kpi-datamovin" \
      org.opencontainers.image.url="https://kf.data.movin.com.ar" \
      org.opencontainers.image.documentation="https://movin.com.ar/product-datos-campo" \
      org.opencontainers.image.vendor="Movin" \
      org.opencontainers.image.version="${DATAMOVIN_IMAGE_VERSION}" \
