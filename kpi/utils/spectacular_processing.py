...
def sanitize_example_credentials(value):
    """Replace credential-shaped example values without changing API schemas."""
    if isinstance(value, dict):
        sanitized = {}
        for key, item in value.items():
            if key in SENSITIVE_EXAMPLE_KEYS and isinstance(item, str):
                sanitized[key] = f'example-{key.replace("_", "-")}'
            else:
                sanitized[key] = sanitize_example_credentials(item)
        return sanitized

    if isinstance(value, list):
        return [sanitize_example_credentials(item) for item in value]
...
def merge_allauth_headless_schema(result, generator, request, public):
    ...
    # in the Swagger UI.
    allauth_schema = sanitize_example_credentials(get_schema())
