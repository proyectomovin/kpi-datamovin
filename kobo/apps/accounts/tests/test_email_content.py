...
class EmailContentModelTestCase(TestCase):
    ...
    def test_custom_activation_email_template_blank_content(self):
        ...
        default = 'Thanks for signing up with DataMovin!'
        ...
        self.client.get(reverse('account_email_verification_sent'))
        assert len(mail.outbox) == 1
        assert mail.outbox[0].subject == 'Activate your DataMovin Account'
        assert email_content.content in mail.outbox[0].body
    ...
    def test_default_activation_email_template(self):
        ...
        default_subject = 'Activate your DataMovin Account'
        default_greeting = 'Welcome to DataMovin!'
        default_body = (
            'To get started, please activate your account by clicking the link below:'
        )
        default_closing = 'You can find DataMovin help and service information here:'
        request = self.client.post(self.signup_url, data)
        user = get_user_model().objects.get(email=email)
        assert request.status_code == status.HTTP_302_FOUND
        self.client.login(username=user.username, password=user.password)
        ...
        assert 'Thanks for signing up!\nThe DataMovin Team' in mail.outbox[0].body
