...
function getLimitReachedMessage(isMmo: boolean, isTeamLabelActive: boolean, limits: string, contactUrl: string) {
  ...
  const secondSentence = t(
    ...
    .replace('##CONTACT_LINK##', contactUrl)
...
// External support links open in a separate tab; in-app account links stay in the same tab.
...
function OverLimitModal(props: OverLimitModalProps) {
  ...
  const contactUrl = envStore.data.support_url || 'https://movin.com.ar/product-datos-campo'
  const limitReachedMessage = getLimitReachedMessage(isMmo, shouldUseTeamLabel, allLimitsText, contactUrl)
