...
const DEFAULT_SUPPORT_EMAIL = 'hola@movin.com.ar'

/**
 * Replaces the DataMovin fallback address with the deployment-specific address
 * from the `/environment` endpoint.
 */
export function replaceSupportEmail(str: string): string {
  if (typeof envStore.data.support_email === 'string' && envStore.data.support_email.length !== 0) {
    return str.replace(DEFAULT_SUPPORT_EMAIL, envStore.data.support_email)
  } else {
    return str
  }
}
